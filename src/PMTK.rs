...
use crate::{GetData, is_valid_checksum};
...
impl SendPmtk for Gps {
    ...
    fn pmtk_414_api_q_nmea_output(&mut self) -> NmeaOutput {
        ...
        return match self.pmtk_500() {
            Some(args) => {
                ...
                let gll: &str = args.get(0).unwrap_or(&"-1");
                let rmc: &str = args.get(1).unwrap_or(&"-1");
                let vtg: &str = args.get(2).unwrap_or(&"-1");
                let gga: &str = args.get(3).unwrap_or(&"-1");
                let gsa: &str = args.get(4).unwrap_or(&"-1");
                let gsv: &str = args.get(5).unwrap_or(&"-1");
    ...
    fn pmtk_607_q_epo_info(&mut self) -> EpoData {
        ...
        EpoData {
            ...
            fwn_ftow__tow: args.get(3).unwrap_or(&"-1").parse::<i8>().unwrap(),
